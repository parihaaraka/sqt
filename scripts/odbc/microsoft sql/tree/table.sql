select
	...
	case when c.is_identity = 1 then ' identity(' + cast(ident_seed('$table.name$') as varchar(20)) + ',' + cast(ident_incr('$table.name$') as varchar(20)) + ')' else '' end +
	case when c.is_nullable = 0 then ' not null' else '' end
	end +
        isnull('<br>&nbsp;&nbsp;<i>default</i> ' + df.definition, '') + '</span>'
        ui_name,
