...
### Keyword case
The `script` result is post-processed when the *Lowercase keywords* option is on
(settings dialog): the keywords and the function names known from the dbms'
`hl.conf` get lowercased. This way the scripts do not have to be maintained in
two cases, and the uppercase-only ddl generated by the dbms itself
(`pg_get_functiondef()`, `pg_get_viewdef()`, etc.) is unified with the
handwritten one.

The very same lexer as the editor's syntax highlighter is used, so the
substitution never touches anything but the keywords: string literals, quoted
identifiers, comments and dollar quoted bodies (routine sources) keep their
original case. Note that a keyword lexeme is lowercased even where it is used as
an identifier (`select count(*) from public.value`), just like it gets colored
as a keyword there.
